...
print (colored(f"Ξ Follow myInstagram: sulthan___i \nΞ START           : {x} \nΞ Your IP         : {ip} ", 'green', attrs=['bold']))
print (colored('••••••••••••••••••••••••••••••••••••••••••••••••••••••••••• \n', 'green', attrs=['bold']))
...
async def levelup(ctx, amount: int):
    ...
    while msgsend > 0:
        try:
            msgsend -= 1
            print(f"{Fore.WHITE}[ {Fore.GREEN}+ {Fore.WHITE}] {Fore.LIGHTBLACK_EX}Message sent! {ctx.channel.history} | Messages left to send: {Fore.WHITE}{msgsend} {Fore.LIGHTBLACK_EX}| Estimated Time: {Fore.WHITE}{scale(msgsend)}")
            if msgsend == 0:
        ...
        async for message in ctx.message.channel.history(limit=1):
            try:
                ...
                print(f"{Fore.WHITE}[ {Fore.RED}- {Fore.WHITE}] {Fore.LIGHTBLACK_EX} Delete message {Fore.WHITE}#{msgsend}")
            except:
                print(f"{Fore.WHITE}[ {Fore.RED}- {Fore.WHITE}] {Fore.LIGHTBLACK_EX}Cannot delete message {Fore.WHITE}#{msgsend}")
                pass
        await asyncio.sleep(10) #setdelayhere(s)
    return
...
async def on_command_error(ctx, error):
    ...
    if isinstance(error, commands.CommandNotFound):
        ...
        print(f"{Fore.WHITE}[ {Fore.RED}E {Fore.WHITE}] {Fore.LIGHTBLACK_EX}Discord error: {error_str} "+Fore.RESET)
